...
export default class Login extends Command {
	...
	async run(): Promise<void> {
		...
		saveConfig({ serverUrl, sessionToken: match[1] });
		...
		if (process.env.CASCADE_SERVER_URL || process.env.CASCADE_SESSION_TOKEN) {
			this.log(
				'Note: CASCADE_SERVER_URL / CASCADE_SESSION_TOKEN env vars will override stored config.',
			);
		}
	}
}
