...
export class GitLabSCMIntegration implements SCMIntegration {
	...
	async hasIntegration(projectId: string): Promise<boolean> {
		...
		const [impl, rev] = await Promise.all([
			getIntegrationCredentialOrNull(projectId, 'scm', 'gitlab', 'implementer_token'),
			getIntegrationCredentialOrNull(projectId, 'scm', 'gitlab', 'reviewer_token'),
		]);
	...
	async hasPersonaToken(projectId: string, persona: 'implementer' | 'reviewer'): Promise<boolean> {
		...
		const token = await getIntegrationCredentialOrNull(projectId, 'scm', 'gitlab', role);
		return token !== null;
	}
	...
	async withCredentials<T>(projectId: string, fn: () => Promise<T>): Promise<T> {
		const token = await getIntegrationCredential(projectId, 'scm', 'gitlab', 'implementer_token');
