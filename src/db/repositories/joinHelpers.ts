 ...
 * - (projectId, workItemId = workItemId) — PM-triggered runs (work-item-only rows)
 *
 ...
 * workItemId but no prNumber) will still match via the second branch because
 * their workItemId = workItemId.
 */
export function buildAgentRunWorkItemJoin(): SQL | undefined {
	return or(
		...
		// Branch 2: Match by workItemId = workItemId (only for PM-triggered runs that have no prNumber)
		...
		and(
			eq(agentRuns.projectId, prWorkItems.projectId),
			sql`${agentRuns.workItemId} = ${prWorkItems.workItemId}`,
