-- Webhook call logs: store last N incoming webhook calls for debugging
...
-- Index for time-based queries (most common: list recent)
CREATE INDEX idx_webhook_logs_received_at ON webhook_logs (received_at DESC);

-- Index for filtering by source
CREATE INDEX idx_webhook_logs_source ON webhook_logs (source);

-- Composite index for source + time filtering
