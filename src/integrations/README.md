...
| New-provider guarantee | Adding a PM provider requires zero edits to `pm-wizard.tsx`, `pm-wizard-common-steps.tsx`, or `pm-wizard-hooks.ts`. Provider-owned discovery hooks, auth metadata, verification display formatting, and webhook UX composition live in the provider folder. |
