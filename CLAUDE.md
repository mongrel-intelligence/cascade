...
- `WORKER_IMAGE_REGISTRY_USERNAME` / `WORKER_IMAGE_REGISTRY_PASSWORD` — router-side registry credentials for worker-image pulls (spawn self-heal, worker-image validation, Dockerfile-build base refresh). Dockerode sends no credentials by default and the Docker daemon has no ambient login, so a **private** worker-image registry 401s on every anonymous pull — e.g. after a host image prune removes the worker image. For ghcr.io, use a PAT with `read:packages` as the password. `WORKER_IMAGE_REGISTRY_SERVER` optionally overrides the registry host derived from the image ref. Unset → anonymous pulls (fine for public images or hosts where the image is guaranteed pre-pulled). See `src/router/registry-auth.ts`.
